...
function genEnemies(){
	for (var i = 0; i < enemyTotal; i++) {
		...
		enemies.push(enemy);
		var randomtile = (Math.floor(Math.random()*8));
		var randomoffset = (Math.floor(Math.random()*30));
		enemy_x = (randomtile*(width/10)) + randomoffset;
	}
}	
...
function genLight(){
	...
	if(randomnumber < 10){
		...
		var randomtile = (Math.floor(Math.random()*10));
		var randomoffset = (Math.floor(Math.random()*30));
		light_sprite.position_x = (randomtile*(width/10)) + randomoffset;
...
function drawShip() {
	if(!disableControls){
		...
		if (ship_sprite.position_x <= -30){
			ship_sprite.position_x = -30;
		} 
		if ((ship_sprite.position_x + ship_sprite.size_w) >= width+30) {
			ship_sprite.position_x = width - ship_sprite.size_w +30;
		}
	...
	if(!disableControls){
		...
		if (lasers.length){
			for (var i = 0; i < lasers.length; i++) {
				...
				ctx.arc(lasers[i].position_x,lasers[i].position_y,10, Math.PI*2,false);
				ctx.fill();
...
function scoreTotal() {
    ...
    ctx.fillText('Click to Play Again', 0, (height / 2) + 35);
...
function startPage() {
	...
	ctx.fillStyle = '#fff';
    ctx.font = 'bold 25px Tele-marines';
	ctx.shadowColor = "white"; // string
	ctx.shadowOffsetX = 0; // integer
	ctx.shadowOffsetY = 0; // integer
	ctx.shadowBlur = 10; // integer
	ctx.textAlign="center";
    ctx.fillText('EliAsteroids', width/2, 100);
    ctx.font = 'bold 15px Fixedsys';
    //ctx.font = 'bold 15px courier';
    ctx.fillText('Touch to Play', width/2, 150);
	...
	ctx.lineWidth="4";
	ctx.strokeStyle="red";
	ctx.rect(0,height/2,width/2,height/4); 
	ctx.stroke();
	ctx.fillText('Up Left',width*1/4-4,(height*5/8)  );

	// Green rectangle
	ctx.beginPath();
	ctx.lineWidth="4";
	ctx.strokeStyle="green";
	ctx.rect(width/2,height/2,width/2,height/4); 
	ctx.stroke();
	ctx.fillText('Up Right',width*3/4,(height*5/8) );

	// Blue rectangle
	ctx.beginPath();
	ctx.lineWidth="4";
	ctx.strokeStyle="blue";
	ctx.rect(0,height*3/4,width/2-4,height/4); 
	ctx.stroke();
	ctx.fillText('Down Left',width*1/4,(height*7/8) );

	// Blue rectangle
	ctx.beginPath();
	ctx.lineWidth="4";
	ctx.strokeStyle="yellow";
	ctx.rect(width/2,height*3/4,width/2,height/4); 
	ctx.stroke();
	ctx.fillText('Down Right',width*3/4,(height*7/8) );
...
function mouseMove(e){
	e.preventDefault();
	mouse_X = e.clientX - document.getElementById('canvas').offsetLeft;
	mouse_Y = e.clientY - document.getElementById('canvas').offsetTop;
}
...
function keyDown(e){
	...
	mouse_downX = parseInt(touchobj.clientX)  - document.getElementById('canvas').offsetLeft;
	mouse_downY = parseInt(touchobj.clientY)  - document.getElementById('canvas').offsetTop;
	block1X = Math.floor(mouse_downX);
	block1Y = Math.floor(mouse_downY);
	if(block1X < width/2){
		rightKey = false;
		leftKey = true;
	}
	...
	if((block1Y < height *3/4)&&(block1Y > height/2)){
		upKey = true;
		downKey = false;
	}
	else{
	...
	// debug = "touch down @ " + block1X + " " + block1Y;
	
	// ctx.clearRect(0,0,320,50);
	// ctx.font = 'bold 15px console';
	// ctx.fillStyle = '#000';
	// var text = "Console";
	// ctx.fillText(debug, 30,0);		
}
function keyUp(e){
	e.preventDefault(); 
	...
	upKey = false;
	downKey = false;
	// debug = "touch up @ " + block1X + " " + block1Y;
